...
public class BetterIdlingResource implements IdlingResource, BetterExecutionBridge {
    ...
    public boolean isIdleNow() {
        ...
        synchronized (idler) {
            if (isLogged)
                Log.i("LOG", "called isidlenow: " + idler, null);
            return idler == 0;
        }
    }
    ...
    public void registerIdleTransitionCallback(ResourceCallback resourceCallback) {
        ...
        if (isLogged)
            Log.i("LOG", "called register Idle: " + idler, null);
    ...
    public void onStart() {
        ...
        synchronized (idler) {
            idler++;
            if (isLogged)
                Log.i("LOG", "called onstart: " + idler, null);
        }
    }
    ...
    public void onError() {
        ...
        synchronized (idler) {
            idler--;
            if (isLogged)
                Log.i("LOG", "called onerrror: " + idler, null);
            if (idler == 0 && cb != null) {
    ...
    public void onEnd() {
        ...
        synchronized (idler) {
            idler--;
            if (isLogged)
                Log.i("LOG", "called onend: " + idler, null);
            if (idler == 0 && cb != null) {
                cb.onTransitionToIdle();
            }
        }
